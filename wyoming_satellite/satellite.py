...
class SatelliteBase:
    ...
    async def trigger_server_connected(self) -> None:
        ...
        _LOGGER.debug("Warming up server (sending warmup pipeline request)...")
        asyncio.create_task(self._warmup_server())

    async def _warmup_server(self) -> None:
        """Send a warmup request to trigger server model loading."""
        try:
            await asyncio.sleep(2)

            _LOGGER.debug("Sending warmup RunPipeline request")
            await self._send_run_pipeline(pipeline_name=None)
            ...
            _LOGGER.debug("Server warmup completed")

        except Exception as e:
            _LOGGER.warning("Failed to warmup server: %s", e)
